	...
	class myCRED_Hooks_Module extends myCRED_Module {
		...
		public function get( $save = false ) {
			...
			$installed['link_click'] = array(
				...
				'description' => str_replace( '%shortcode%', '<a href="#">mycred_link</a>', __( 'Award %_plural% for clicks on links generated by the %shortcode% shortcode.', 'mycred' ) ),
			...
			$installed['video_view'] = array(
				...
				'description' => str_replace( '%shortcode%', '<a href="#">mycred_video</a>', __( 'Award %_plural% for watches videos embedded using the %shortcode% shortcode.', 'mycred' ) ),
...
#sidebar-active .widget-inside form .widget-content ol li span.description { display: block; font-style: italic; font-size: 12px; line-height: 16px; padding-left: 0; padding-right: 0; }
...
<div class="wrap">
	<h1><?php echo sprintf( __( '%s Hooks %s', 'mycred' ), mycred_label(), $button ); ?></h1>
							...
							<div class="widget-inside">
								<form method="post" action="">
									...
									<div class="widget-control-actions">
										<div class="alignleft">
											<a class="widget-control-remove" href="#remove"><?php _e( 'Delete', 'mycred' ); ?></a> | <a class="widget-control-close" href="#close"><?php _e( 'Close', 'mycred' ); ?></a>
										</div>
...
<?php
			...
			// Role based setup
			elseif ( $this->setup == 'role' ) {

				$roles           = $this->get_roles();
				$number_of_roles = count( $roles );
				$per_column      = ceil( $number_of_roles / 2 );
				$columns         = array_chunk( $roles, $per_column, true );

?>
<div id="widgets-right">
<?php

				foreach ( $columns as $column_num => $role ) {

?>
	<div class="sidebars-column-<?php echo $column_num + 1; ?>">

		<?php foreach ( $role as $role_id => $role_info ) : ?>

		<div class="widgets-holder-wrap">
			<div id="sidebar-<?php echo esc_attr( $role_id ); ?>" class="widgets-sortables ui-droppable ui-sortable">
				<div class="sidebar-name">
					<div class="sidebar-name-arrow"><br /></div>
					<h2><?php echo esc_attr( $role_info['name'] ); ?></h2>
				</div>
				<div class="sidebar-description">
					<p class="description"><?php printf( __( 'The following hooks are used for %s.', 'mycred' ), esc_attr( $role_info['name'] ) ); ?></p>
				</div>
...
<?php
		...
		protected function get_roles() {
			...
			if ( ! empty( $wp_roles ) ) {
				foreach ( $wp_roles as $role_id => $role_setup ) {

					$capabilities = $role_setup['capabilities'];

					// If we are excluding admins, the role does not need to be included
					if ( $this->core->exclude['plugin_editors'] == 1 && array_key_exists( $this->core->caps['plugin'], $capabilities ) && $capabilities[ $this->core->caps['plugin'] ] == 1 )
						continue;

					// If we are excluding editors, the role does not need to be included
					if ( $this->core->exclude['cred_editors'] == 1 && array_key_exists( $this->core->caps['creds'], $capabilities ) && $capabilities[ $this->core->caps['creds'] ] == 1 )
						continue;

					$available_roles[ $role_id ] = $role_setup;
					...
					if ( $sidebar_id == 'sidebar-active' ) {
						...
						if ( ! empty( $this->active ) && ! empty( $clean_hook_ids ) ) {
							foreach ( $this->active as $already_active_hook_id ) {
								...
								// If the hook ID no longer part of the hook we have disabled it.
								elseif ( ! in_array( $already_active_hook_id, $clean_hook_ids ) ) {
									$this->call( 'deactivate', $installed[ $already_active_hook_id ]['callback'], $this->hook_prefs[ $already_active_hook_id ] );
								}
...
if ( ! class_exists( 'myCRED_Hook_Video_Views' ) ) :
	class myCRED_Hook_Video_Views extends myCRED_Hook {
		...
		public function ajax_call_video_points() {
			...
			// Get user id
			$user_id  = get_current_user_id();
			if ( $this->core->exclude_user( $user_id ) ) die( 'EXCLUDED' );
...
<ol>
	<li><span class="description"><?php echo $this->core->template_tags_general( __( 'Select when %_plural% should be awarded or deducted.', 'mycred' ) ); ?></span></li>
	<li>
		<label for="<?php echo $this->field_id( array( 'logic' => 'play' ) ); ?>"><input type="radio" name="<?php echo $this->field_name( 'logic' ); ?>" id="<?php echo $this->field_id( array( 'logic' => 'play' ) ); ?>"<?php checked( $prefs['logic'], 'play' ); ?> value="play" /> <?php _e( 'Play - As soon as video starts playing.', 'mycred' ); ?></label>
		<label for="<?php echo $this->field_id( array( 'logic' => 'full' ) ); ?>"><input type="radio" name="<?php echo $this->field_name( 'logic' ); ?>" id="<?php echo $this->field_id( array( 'logic' => 'full' ) ); ?>"<?php checked( $prefs['logic'], 'full' ); ?> value="full" /> <?php _e( 'Full - First when the entire video has played.', 'mycred' ); ?></label>
		<label for="<?php echo $this->field_id( array( 'logic' => 'interval' ) ); ?>"><input type="radio" name="<?php echo $this->field_name( 'logic' ); ?>" id="<?php echo $this->field_id( array( 'logic' => 'interval' ) ); ?>"<?php checked( $prefs['logic'], 'interval' ); ?> value="interval" /> <?php echo $this->core->template_tags_general( __( 'Interval - For each x number of seconds watched.', 'mycred' ) ); ?></label>
	</li>
</ol>
...
<label class="subheader"><?php _e( 'Referring Signups', 'mycred' ); ?></label>
<ol>
	<li><span class="description"><?php _e( 'Registrations are disabled.', 'mycred' ); ?></span>
...
<ol>
	<li>
		<div class="h2"><input type="text" name="<?php echo $this->field_name( array( 'setup' => 'IP' ) ); ?>" id="<?php echo $this->field_id( array( 'setup' => 'IP' ) ); ?>" value="<?php echo absint( $prefs['setup']['IP'] ); ?>" size="8" /></div>
...
<ol>
	...
	<li>
		...
		<div class="h2"><input type="text" name="<?php echo $this->field_name( array( 'buddypress' => 'title' ) ); ?>" id="<?php echo $this->field_id( array( 'buddypress' => 'title' ) ); ?>" value="<?php echo esc_attr( $prefs['buddypress']['title'] ); ?>" class="long" /></div>
...
?>
	...
	<li>
		...
		<div class="h2"><input type="text" name="<?php echo $this->field_name( array( 'buddypress' => 'priority' ) ); ?>" id="<?php echo $this->field_id( array( 'buddypress' => 'priority' ) ); ?>" value="<?php echo absint( $prefs['buddypress']['priority'] ); ?>" size="8" /></div><br />
...
<input type="hidden" name="<?php echo $this->field_name( array( 'buddypress' => 'title' ) ); ?>" id="<?php echo $this->field_id( array( 'buddypress' => 'title' ) ); ?>" value="<?php echo esc_attr( $prefs['buddypress']['title'] ); ?>" />
