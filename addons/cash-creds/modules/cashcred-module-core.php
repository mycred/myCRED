	...
	class myCRED_cashCRED_Module extends myCRED_Module {
		...
		public function cashcred_pay_now( $post_id = false, $auto = false ) {
			...
			if ( $cashcred_pay_method !== false && array_key_exists( $cashcred_pay_method, $cashcred_instance->active ) ) {
				...
				do_action( 'mycred_cashcred_process',$cashcred_pay_method, get_option( 'mycred_pref_cashcreds' , false ) );
				do_action( "mycred_cashcred_process_{$cashcred_pay_method}", get_option( 'mycred_pref_cashcreds' , false ) );
		...
		public function cashcred_developer_log () {
			...
			$counter = (int)get_post_meta( $this->post_id, 'cashcred_log_counter', true );
			$orderdesc = $counter;
			...
			for ($log = 1; $log <= $counter; $log++) {
				...
				$payment_log = get_post_meta( $this->post_id, 'cashcred_log_' . $orderdesc, true );
				...
				$logdata .=  print_r( json_decode( $payment_log["Paypal_response"] ) , true );
				$logdata .= "</pre>";
		...
		public function cashcred_update_payment_status( $post_id , $manual ) {
			...
			mycred_subtract( 'Cashcred Withdrawal', $user_id, -$points, $entry, $post_id , $log_data, $point_type );
		...
		public function process_new_withdraw_request( $gateway_id ){
			...
			$requested_url 		   = $_SERVER['REQUEST_SCHEME'] . '://' . $_SERVER['HTTP_HOST'].$_SERVER['REQUEST_URI'];	 
			...
			if ( $post_id !== NULL && ! is_wp_error( $post_id ) ) {
				...
				if( isset($mycred_pref_cashcreds['gateway_prefs'][$cashcred_instance->gateway_id]["allow_auto_withdrawal"]) &&  
					$mycred_pref_cashcreds['gateway_prefs'][$cashcred_instance->gateway_id]["allow_auto_withdrawal"] ==="yes" ){
