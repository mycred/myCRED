...
if ( ! class_exists( 'cashCRED_Pending_Payments' ) ) :
	class cashCRED_Pending_Payments extends myCRED_Module {
		...
		public function cashcred_filter_html() {
			...
			if ( $post_type == MYCRED_CASHCRED_KEY ) {  
				...
				<select name="Status" id="Status">
					<option value="all" <?php selected( 'all', $current_plugin ); ?>><?php _e( 'All Status', 'mycred' ); ?></option>
				...
				<select name="gateway" id="gateway">
					<option value="all" <?php selected( 'all', $current_selected ); ?>><?php _e( 'All Gateway', 'mycred' ); ?></option>
					<?php 
					...
					<?php echo esc_attr( $info['title'] ); ?></option>
					<?php } ?>
				</select>
			   <?php 
				$current_user = '';
				...
				<select name="user_id" id="user_id">
					<option value="all" <?php selected( 'all', $current_user ); ?>><?php _e( 'All Users', 'mycred' ); ?></option>
					<?php 
					foreach ( $users as $user ) { ?> 
					<option value="<?php echo esc_attr( $user->ID ); ?>" <?php selected( $user->ID, $current_user ); ?>>
					<?php echo esc_attr( $user->user_nicename ); ?></option>
					<?php } ?>
				</select>
			<?php 
			}
		...
		public function add_metaboxes() {
			...
			//if ( get_post_meta( get_the_ID(), 'cashcred_deve_log', true ) == 'on' )
				
			
			
			$mycred_pref_cashcreds = get_option( 'mycred_pref_cashcreds' , false );
			
			
			if(isset($mycred_pref_cashcreds["gateway_prefs"]["paypal-standard"]["cashcred_deve_log"]) && 
			 $mycred_pref_cashcreds["gateway_prefs"]["paypal-standard"]["cashcred_deve_log"] == 'on'
			){
		...
		public function cashcred_developer_log () {
			...
			$counter = (int)get_post_meta( get_the_ID(), 'cashcred_log_counter', true );
			$orderdesc = $counter;
			...
			for ($log = 1; $log <= $counter; $log++) {
				...
				$payment_log = get_post_meta( get_the_ID(), 'cashcred_log_' . $orderdesc, true );
				...
				echo 	print_r(json_decode($payment_log["Paypal_response"]),true);	
				echo "</pre>";
		...
		foreach ( $mycred_modules['solo']['cashcred']->get() as $gateway_id => $info ) {
			...
			/// change orgnal function suffian 
		...
		public function cashcred_payment_pay(){
			...
			<?php //  if ( array_key_exists ( "bank" , $cashcred_user_settings ) ) { ?>
			...
			<div class="row">
				...
				<div class="col-md-4 col-sm-12">
					<div class="form-group"> <span class="entry-date">
					<?php
			...
			<script type="text/javascript">
				...
				jQuery.ajax({
					...
					success: function(response) {
						...
						if(jQuery('#cashcred-developer-log').length) {
...
<?php
		...
		public function metabox_cashcred_comments( $post ) {
			...
			foreach ( $comments as $comment ) {
				...
				echo '<li><time>' . $comment->comment_date_gmt . '</time><p>' . $comment->comment_content . '</p></li>';
		...
		public function save_pending_payment( $post_id, $post ) {
			...
			mycred_update_post_meta( $post_id, 'status', $_POST['status'] );
			update_user_meta( $_POST['user_id'], 'cashcred_user_settings', $_POST['cashcred_user_settings'] );
			...
			if ( $changed ) {
				...
				$this->add_comment( $post_id, sprintf( __( 'Withdrawal payment updated by %s', 'mycred' ), $user->user_login ) );
			}
