...
<?php
		...
		public function list_option_countries( $selected = '' ) {
			...
			foreach ( $countries as $code => $cname ) {
				echo '<option value="' . esc_attr( $code ) . '" ' . selected( $selected, $code ) . '>' . esc_html( $cname ) . '</option>';
			}
		...
		public function list_option_months( $selected = '' ) {
			...
			foreach ( $months as $number => $text ) {
				echo '<option value="' . esc_attr( $number ) . '" ' . selected( $selected, $number ) . '>' . esc_html( $text ) . '</option>';
			}
