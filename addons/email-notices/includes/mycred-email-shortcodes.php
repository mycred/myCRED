...
if (! function_exists('mycred_render_email_subscriptions') ) :
    function mycred_render_email_subscriptions( $atts = array(), $content = '' )
    {
        ...
        $email_notices   = $wpdb->get_results(
            $wpdb->prepare(
                "
			SELECT * 
...
<form action="<?php echo esc_url($url); ?>" id="mycred-email-subscriptions" method="post">
    <table class="table">
        ...
        <?php if (! empty($email_notices) ) : ?>
            ...
            <tr>
                <td class="check"><input type="checkbox" name="mycred_email_unsubscribe[]"<?php if (in_array($notice->ID, $unsubscriptions) ) { echo ' checked="checked"'; 
                                                                                          } ?> value="<?php echo $notice->ID; ?>" /></td>
