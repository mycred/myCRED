...
if ( ! class_exists( 'myCRED_Payment_Gateway' ) ) :
	abstract class myCRED_Payment_Gateway {
		...
		public function valid_request() {
			...
			if ( ! isset( $_REQUEST['token'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_REQUEST['token'] ) ), 'mycred-buy-creds' ) ) return false;
			...
			$this->transaction_id = ( isset( $_REQUEST['revisit'] ) ) ? strtoupper( sanitize_text_field( wp_unslash( $_REQUEST['revisit'] ) ) ) : false;
	...
	<tr>
		...
		public function add_pending_payment( $data ) {
			...
			if ( isset( $_REQUEST['transaction_id'] ) )
				$post_title = sanitize_title( wp_unslash( $_REQUEST['transaction_id'] ) );
			else
		...
		public function get_amount() {
			...
			if ( isset( $_REQUEST['amount'] ) && is_numeric( $_REQUEST['amount'] ) ) {
				...
				$amount  = $this->core->number( sanitize_text_field( wp_unslash( $_REQUEST['amount'] ) ) );
