...
<div id="mycred-payment-option-dv" class="payment-option-dv">
	<a id="mycred-payment-option-lnk" class="payment-option-lnk algn-vrt display-the-hidden" rel="mycred-payment-option-form" style="display: table-cell">
		<div class="vrt-cell">
			<div><?php echo esc_html( $this->prefs['labels']['gateway'] ); ?></div>
		</div>
	</a><br/>
	...
	<?php if ( $member && $solvent ) : ?>
		...
		<p><?php echo esc_html( $this->core->template_tags_general( $this->prefs['messages']['solvent'] ) ); ?></p>

		<?php endif; ?>

		<div class="event-display-boxes">
			<h4 id="mycred_title" class="payment_type_title section-heading"><?php echo esc_html( $this->prefs['labels']['payment'] ); ?></h4>
			<table style="width:100%;">
				<tr>
					<td class="info"><?php esc_html_e( 'Current Balance', 'mycred' ); ?></td>
					<td class="amount"><?php echo esc_html( $this->core->format_creds( $balance ) ); ?></td>
				</tr>
				<tr>
					<td class="info"><?php esc_html_e( 'Total Cost', 'mycred' ); ?></td>
					<td class="amount"><?php echo esc_html( $this->core->format_creds( $event_cost ) ); ?></td>
				</tr>
				<tr>
					<td class="info"><?php esc_html_e( 'Balance After Purchase', 'mycred' ); ?></td>
					<td class="amount"><?php echo esc_html( $this->core->format_creds( $after_purchase ) ); ?></td>
				</tr>
			</table>
			<p><a href="<?php echo esc_url( $finalize_link ); ?>" class="button button-large button-primary" style="float:right;"><?php echo esc_html( $this->prefs['labels']['button'] ); ?></a></p>
		</div>
	...
	<?php elseif ( $member && ! $solvent ) : ?>
		...
		<div class="event_espresso_attention event-messages ui-state-highlight">
			...
			<p><?php echo esc_html( $this->core->template_tags_general( $this->prefs['messages']['insolvent'] ) ); ?></p>
		</div>
		<div class="event-display-boxes">
			<h4 id="mycred_title" class="payment_type_title section-heading"><?php echo esc_html( $this->prefs['labels']['payment'] ); ?></h4>
			<table style="width:100%;">
				...
				<tr class="after-purchase">
					<td class="info"><?php esc_html_e( 'Balance After Purchase', 'mycred' ); ?></td>
					<td class="amount" style="color:red;"><?php echo esc_html( $this->core->format_creds( $after_purchase ) ); ?></td>
				</tr>
			</table>
		</div>
...
<?php else : ?>
		...
		<div class="event_espresso_attention event-messages ui-state-highlight">
			<span class="ui-icon ui-icon-alert"></span>
			<p><?php echo esc_html( $this->core->template_tags_general( $this->prefs['messages']['visitors'] ) ); ?></p>
		</div>
...
<div class="metabox-holder">
	<div class="postbox <?php echo esc_html( $postbox_style ); ?>">
		<div title="Click to toggle" class="handlediv"><br /></div>
		<h3 class="hndle">
			<?php echo esc_html( $this->label ) . ' ' . esc_html__( 'Gateway Settings', 'mycred' ); ?>
		</h3>
		<div class="inside">
			<div class="padding">
				<ul>
<?php
			...
			if ( array_key_exists( 'mycred', $active_gateways ) ) {
				...
				echo '<li id="deactivate_check" style="width:30%;" onclick="location.href=\'' . esc_url( $deactivate_url ) . '\';" class="red_alert pointer"><strong>' . esc_url( $deactivate_text ) . '</strong></li>';
				$this->gateway_settings();

			}
			else {

				echo '<li id="activate_check" style="width:30%;" onclick="location.href=\'' . esc_url( $activate_url ) . '\';" class="green_alert pointer"><strong>' . esc_url( $activate_text ) . '</strong></li>';
...
<form method="post" action="<?php echo isset( $_SERVER['REQUEST_URI'] ) ? esc_url_raw( wp_unslash( $_SERVER['REQUEST_URI'] ) ) : ''; ?>#mycred-gate">
	...
	<table width="99%" border="0" cellspacing="5" cellpadding="5">
		<tr>
			<td valign="top">
				<h4><?php esc_html_e( 'Labels', 'mycred' ); ?></h4>
				<ul>
					<li>
						<label for="mycred-prefs-gateway-labels"><?php esc_html_e( 'Gateway Title', 'mycred' ); ?></label>
						<input type="text" name="mycred_prefs[labels][gateway]" id="mycred-prefs-gateway-labels" size="30" value="<?php echo esc_attr( $this->prefs['labels']['gateway'] ); ?>" /><br />
						<span class="description"><?php esc_html_e( 'Title to show on Payment page', 'mycred' ); ?>.</span>
					</li>
					<li>
						<label for="mycred-prefs-payment-labels"><?php esc_html_e( 'Payment Type', 'mycred' ); ?></label>
						<input type="text" name="mycred_prefs[labels][payment]" id="mycred-prefs-payment-labels" size="30" value="<?php echo esc_attr( $this->prefs['labels']['payment'] ); ?>" /><br />
						<span class="description"><?php esc_html_e( 'Title to show on receipts and logs', 'mycred' ); ?>.</span>
					</li>
					<li>
						<label for="mycred-prefs-button-labels"><?php esc_html_e( 'Button Label', 'mycred' ); ?></label>
						<input type="text" name="mycred_prefs[labels][button]" id="mycred-prefs-button-labels" size="30" value="<?php echo esc_attr( $this->prefs['labels']['button'] ); ?>" /><br />
						<span class="description"><?php esc_html_e( 'Pay Button', 'mycred' ); ?></span>
					</li>
				</ul>
				...
				<ul>
					<li>
						<label for="mycred-prefs-payment-type"><?php esc_html_e( 'Point Type', 'mycred' ); ?></label>
				...
				<h4><?php esc_html_e( 'Price', 'mycred' ); ?></h4>
				<ul>
					<li id="mycred-event-exchange-box">
						<label for="mycred-prefs-price-x-rate"><?php esc_html_e( 'Exchange Rate', 'mycred' ); ?></label>
						<input type="text" name="mycred_prefs[rate]" id="mycred-prefs-price-x-rate" size="30" value="<?php echo esc_attr( $this->prefs['rate'] ); ?>" /><br />
						<span class="description"><?php echo esc_html( $exchange_message ); ?></span>
					</li>
					<li>
						<p><strong><?php esc_html_e( 'Important!', 'mycred' ); ?></strong></p>
						<ol>
							<li><?php esc_html_e( 'You can disable purchases using this gateway by adding a custom Event Meta: <code>mycred_no_sale</code>', 'mycred' ); ?></li>
							<li><?php esc_html_e( 'Users must be logged in to use this gateway!', 'mycred' ); ?></li>
						</ol>
					</li>
					<li id="mycred-event-profit-sharing">
						<label for="mycred-prefs-profit-share"><?php esc_html_e( 'Profit Sharing', 'mycred' ); ?></label>
						<input type="text" name="mycred_prefs[share]" id="mycred-prefs-profit-share" size="5" value="<?php echo esc_attr( $this->prefs['share'] ); ?>" /> %<br />
						<span class="description"><?php esc_html_e( 'Option to share sales with the product owner. Use zero to disable.', 'mycred' ); ?></span>
					</li>
				</ul>
				<h4><?php esc_html_e( 'Log', 'mycred' ); ?></h4>
				<ul>
					<li>
						<label for="mycred-prefs-log"><?php esc_html_e( 'Log Entry', 'mycred' ); ?></label>
						<input type="text" name="mycred_prefs[log]" id="mycred-prefs-log" size="30" value="<?php echo esc_attr( $this->prefs['log'] ); ?>" /><br />
						<span class="description"><?php echo esc_html( $this->core->available_template_tags( array( 'general' ) ) ); ?></span>
					</li>
				</ul>
			</td>
			<td valign="top">
				<h4><?php esc_html_e( 'Templates', 'mycred' ); ?></h4>
				<ul>
					<li>
						<label for="mycred-prefs-message-solvent"><?php esc_html_e( 'Solvent users', 'mycred' ); ?></label>
						<textarea name="mycred_prefs[messages][solvent]" id="mycred-prefs-message-solvent" style="width: 90%; max-width: 90%; min-height: 90px;"><?php echo esc_html( stripslashes( $this->prefs['messages']['solvent'] ) ); ?></textarea><br />
						<span class="description"><?php esc_html_e( 'Message to show users on the payment page before they are charged. Leave empty to hide.', 'mycred' ); ?><br /><?php echo esc_html( $this->core->available_template_tags( array( 'general' ) ) ); ?></span>
					</li>
					<li>
						<label for="mycred-prefs-message-insolvent"><?php esc_html_e( 'Insolvent users', 'mycred' ); ?></label>
						<textarea name="mycred_prefs[messages][insolvent]" id="mycred-prefs-message-solvent" style="width: 90%; max-width: 90%; min-height: 90px;"><?php echo esc_html( stripslashes( $this->prefs['messages']['insolvent'] ) ); ?></textarea><br />
						<span class="description"><?php esc_html_e( 'Message to show users who do not have enough points to pay.', 'mycred' ); ?><br /><?php echo esc_html( $this->core->available_template_tags( array( 'general' ) ) ); ?></span>
					</li>
					<li>
						<label for="mycred-prefs-message-insolvent"><?php esc_html_e( 'Visitors', 'mycred' ); ?></label>
						<textarea name="mycred_prefs[messages][visitors]" id="mycred-prefs-message-visitors" style="width: 90%; max-width: 90%; min-height: 90px;"><?php echo esc_html( stripslashes( $this->prefs['messages']['visitors'] ) ); ?></textarea><br />
						<span class="description"><?php esc_html_e( 'Message to show visitors (users not logged in) on the payment page.', 'mycred' ); ?><br /><?php echo esc_html( $this->core->available_template_tags( array( 'general' ) ) ); ?></span>
					</li>
				</ul>
			</td>
		</tr>
	...
	<input type="hidden" name="mycred-gateway-token" value="<?php echo esc_attr( wp_create_nonce( 'mycred-espresso-update' ) ); ?>" />
	<p><input class="button-primary" type="submit" name="Submit" value="<?php esc_attr_e( 'Update Settings', 'mycred' ); ?>" /></p>
</form>
<?php
