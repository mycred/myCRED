    ...
    class myCRED_Remote
    {
        ...
        public function get_host_IP()
        {
            ...
            if (isset($_SERVER['HTTP_CLIENT_IP']) ) {
                $this->host = sanitize_text_field($_SERVER['HTTP_CLIENT_IP']);

            } elseif (isset($_SERVER['HTTP_X_FORWARDED_FOR']) ) {
                $this->host = sanitize_text_field($_SERVER['HTTP_X_FORWARDED_FOR']);

            } elseif (isset($_SERVER['HTTP_X_FORWARDED']) ) {
                $this->host = sanitize_text_field($_SERVER['HTTP_X_FORWARDED']);

            } elseif (isset($_SERVER['HTTP_FORWARDED_FOR']) ) {
                $this->host = sanitize_text_field($_SERVER['HTTP_FORWARDED_FOR']);

            } elseif (isset($_SERVER['HTTP_FORWARDED']) ) {
                $this->host = sanitize_text_field($_SERVER['HTTP_FORWARDED']);

            } elseif (isset($_SERVER['REMOTE_ADDR']) ) {
                $this->host = sanitize_text_field($_SERVER['REMOTE_ADDR']);
...
<ol id="myCRED-actions-cred">
    <li>
        <input type="checkbox" name="mycred_pref_core[allow_remote]" id="myCRED-General-remote"<?php if ($settings['enabled'] ) { echo ' checked="checked"';
} echo $block; ?> value="1" /> <?php if (! empty($disabled_message) ) { ?><span class="description"><?php echo $disabled_message; ?></span><?php 
                                                                                               } ?></li>
</ol>
        <?php
...
if (! function_exists('mycred_remote_settings_page') ) :
    function mycred_remote_settings_page( $mycred_general )
    {
        ...
        ?>
<h4><span class="dashicons dashicons-cloud <?php if (empty($settings['key']) ) { echo 'static'; 
} else { echo 'active';
                                           } ?>"></span><?php _e('Remote Access', 'mycred'); ?></h4>
<div class="body" style="display:none;">
    ...
    <ol id="myCRED-remote-api-key" class="inline">
        ...
        <li>
            ...
            <div class="h2" style="line-height: 30px; color:<?php if ($key_length == 0 ) { echo 'gray'; 
} elseif ($key_length >= 12 ) { echo 'green';
                                                            } ?>">(<span id="mycred-length-counter"><?php echo $key_length; ?></span>)</span></div>
        </li>
        <li>
            <label>&nbsp;</label><br />
    ...
    <ol id="myCRED-remote-api-debug-mode">
        <li>
            <input type="checkbox" name="mycred_pref_core[remote][debug]" id="myCRED-remote-debug-mode"<?php if ($settings['debug'] ) { echo ' checked="checked"';
} ?> value="1" /> <span class="description"><?php _e('Remember to disable when not used to prevent mischievous calls from learning about your setup!', 'mycred'); ?></span></li>
    </ol>
